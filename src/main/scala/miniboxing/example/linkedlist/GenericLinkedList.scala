...
class GenericLinkedList[T](implicit manifest: Manifest[T]) {

  private var amount: Int = 0
  private val first: GenericNode[T] = new GenericNode[T](null.asInstanceOf[T], null)
  private var last: GenericNode[T] = first
  ...
  def map[U](f: T => U)(implicit mu: Manifest[U]) = {
    val linkedList = new GenericLinkedList[U]
    var nodeInit = first
    var nodeNew = linkedList.first
    while (nodeInit.next != null) {
      nodeInit = nodeInit.next
      nodeNew.next = new GenericNode[U](f(nodeInit.elem), null)
  ...
  def filter(f: T => Boolean) = {
    val linkedList = new GenericLinkedList[T]
    var nodeInit = first
    var nodeNew = linkedList.first
    while (nodeInit.next != null) {
      nodeInit = nodeInit.next
      if (f(nodeInit.elem)) {
        nodeNew.next = new GenericNode[T](nodeInit.elem, null)
        nodeNew = nodeNew.next
      }
    }
    linkedList.last = nodeNew
  ...
  private def getNode(i: Int): GenericNode[T] = {
    if (i < 0) {
      throw new NoSuchElementException
    }
    var node = first
  ...
  private def addAfter(t: T, n: GenericNode[T]): Unit = {
    val node = new GenericNode[T](t, n.next)
    if (last eq n) {
      last = node
    }
    node.next = n.next
